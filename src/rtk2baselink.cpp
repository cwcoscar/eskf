...
class Baselink{
    private:
        ...
        ros::Time now_;
    public:
        Baselink(ros::Publisher &pub_b, std::string &rtk_topic, Eigen::Vector3d &rtk_b);
...
Baselink::Baselink(ros::Publisher &pub_b, std::string &rtk_topic, Eigen::Vector3d &rtk_b)
:pub_b_(pub_b), rtk_topic_(rtk_topic), rtk_b_(rtk_b){}
...
void Baselink::GNSSattcallback(const ublox_msgs::NavATT& msg){ 
    ...
    Eigen::Matrix3d R_b_l = coordinate_mat_transformation::Rotation_matrix(att);
...
int main(int argc, char **argv) {
    ...
    // pub_b_.reset(new ros::Publisher);
    // *pub_b_ = n.advertise<uwb_ins_eskf_msgs::uwbFIX>(rtk_topic + "_baselink", 1);
    ros::Publisher pub_b = n.advertise<uwb_ins_eskf_msgs::uwbFIX>(rtk_topic + "/baselink", 1);

    Baselink rtk2baselink(pub_b, rtk_topic, rtk_b);
